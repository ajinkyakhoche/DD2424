...
class kLayerNN(object):
    ...
    def PerformanceEval(self, grad_W, grad_b, numgrad_W, numgrad_b):
        ...
        rel_b = []
        for i in range(self.nLayers):
            a = np.maximum(self.eps, (abs(grad_W[i]) + abs(numgrad_W[i])))
            rel_W.append(np.divide(abs(grad_W[i] - numgrad_W[i]),
                                   np.maximum(self.eps, (abs(grad_W[i]) + abs(numgrad_W[i])))))
            rel_b.append(np.divide(abs(grad_b[i] - numgrad_b[i]),
                                   np.maximum(self.eps, (abs(grad_W[i]) + abs(numgrad_W[i])))))
        return
    ...
    def FinalTest(self):
        '''
        Set the best learning rate and Regularization.
        Run Mini batch grad descent with batch 1-5 as training, 
        batch 5 (last 1000 images as validation) and test batch 
        as testing set and compute accuracy on test set 
        '''
        self.eta = 0.005
        self.lmbda = 1e-8
        [Wstar, bstar] = self.MiniBatchGD(
            self.Xtrain, self.Ytrain, self.Xval, self.Yval, self.W, self.b, PlotLoss=True)

        # Compute Accuracy
        acc = self.ComputeAccuracy2(self.Xtest, self.ytest, Wstar, bstar)
        print('\nAccuracy obtained with best hyperparameters is :' + str(acc))
