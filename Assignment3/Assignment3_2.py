...
class kLayerNN(object):
    ...
    def BatchNormBackPass(self, g, s, mu, var):
        ...
        '''
        N = g.shape[0]
        Vb = np.reshape(np.diag(var + self.epsilon), (-1, 1))
        B = np.reshape(np.diag(s - mu), (-1, 1))
        A = np.multiply(g, Vb**-1.5)
        dJdvar = np.sum(-0.5 * np.multiply(A, B), 0)
    ...
    def EvaluateClassifier2(self, x, Wt, bias):
        ...
        for i in range(self.nLayers-1):
            ...
            var.append(np.reshape(np.sum(((s[i+1]-mu[i+1])**2), 1)/N, (-1, 1)))
