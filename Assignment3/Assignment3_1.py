...
class kLayerNN(object):
    ...
    def BatchNormalize(self, s, mu, var):
        A = (np.diag(var + self.epsilon))**-0.5
        B = s - mu
        sHat = np.multiply(A, B)
        return sHat
    ...
    def BatchNormBackPass(self, g, s, mu, var):
        ...
        '''
        N = g.shape[0]
        Vb = np.reshape(np.diagonal(var + self.epsilon), (-1, 1))
        A = Vb**-1.5
        B = np.reshape(np.diagonal(s - mu), (-1, 1))
        C = np.multiply(g, A)
        D = -0.5 * np.multiply(C, B)
        dJdvar = np.sum(D, 0)
        E = np.multiply(g, Vb**-0.5)
        dJdmu = np.sum(-E, 0)
        X = E
        Y1 = np.reshape(np.diagonal(s - mu), (-1, 1))
        Y = np.multiply(dJdvar, Y1) * 2/N
        Z = dJdmu/N
        g1 = X + Y + Z
        return g1
    ...
    def EvaluateClassifier2(self, x, Wt, bias):
        ...
        for i in range(self.nLayers):
            s.append(np.dot(Wt[i], h[i]) + bias[i])
            # calculate mu and variance
            mu.append(np.sum(s[i+1], 0)/N)
            var.append((s[i+1]+mu[i+1])**2/N)
    ...
    def ComputeCost2(self, X, Y, Wt, bias):
        N = X.shape[1]
        p, _, _, _, _ = self.EvaluateClassifier2(X, Wt, bias)
    ...
    def PerformanceEval(self, grad_W, grad_b, numgrad_W, numgrad_b):
        ...
        abs_b = []
        for i in range(self.nLayers):
            abs_W.append(abs(np.array(grad_W[i]) - np.array(numgrad_W[i])))
            abs_b.append(abs(np.array(grad_b[i]) - np.array(numgrad_b[i])))
        ...
        rel_b = []
        for i in range(self.nLayers):
            rel_W.append(np.sum(abs(grad_W[i] - numgrad_W[i])) /
                         max(self.eps, np.sum(abs(grad_W[i]) + abs(numgrad_W[i]))))
            rel_b.append(np.sum(abs(grad_b[i] - numgrad_b[i])) /
                         max(self.eps, np.sum(abs(grad_b[i]) + abs(numgrad_b[i]))))
    ...
    def ComputeGradients(self, X, Y, Wt, bias):
        ...
        grad_b = []
        grad_b.append([])
        grad_W.append(np.zeros((list(self.m)[0], Wt[0].shape[1])))
        ...
        # Forward Pass
        p, s, h, mu, var = self.EvaluateClassifier2(X, Wt, bias)
        ...
        g = - (Y - p).T
        grad_b[-1] = np.sum(g.T, 1)/N
        grad_b[-1] = np.reshape(grad_b[-1], (-1, 1))
        grad_W[-1] = (np.dot(g.T, h[-2].T))/N + 2 * self.lmbda * Wt[-1]
        # Propogate gradient vector gi to previous layer:
        g = np.dot(g, Wt[-1])
        sTemp = np.where(s[-2] > 0, 1, 0)
        g = np.multiply(g, np.reshape(np.diag(sTemp), (-1, 1)))

        for i in range(self.nLayers-2, -1, -1):
            g = self.BatchNormBackPass(g, s[i+1], mu[i+1], var[i+1])
            grad_b[i] = np.sum(g.T, 1)/N
            grad_b[i] = np.reshape(grad_b[i], (-1, 1))
            grad_W[i] = (np.dot(g.T, h[i].T))/N + 2 * self.lmbda * Wt[i]
            # Propagate gradient vector to previous layer (if i > 0):
            if i > 0:
                g = np.dot(g, Wt[i])
...
def main():
    ...
    GradDescentParams = {'sigma': 0.001, 'eta': 0.01,
                         'lmbda': 0.0, 'rho': 0.9, 'nEpoch': 50, 'nBatch': 100, 'epsilon': 1e-9}
    ...
    '''
    NNParams = {'d': 3072, 'k': 10, 'n': 10000, 'm': {50}}
    ...
    '''
    X_DIM = obj.d  # can change this to 100, 1000 or obj.d (= 3072) etc
    Temp_Wt = []
